...
mutate(ab_sf = list(
  plot_heading %>% 
  get_shp_name(ffy, "TrialDesign", .) %>% 
  st_read() %>% 
  make_sf_utm() 
)) %>% 
mutate(ab_sf = list(
  if ("POINT" %in% st_geometry(ab_sf)) {
    make_ab_line(ab_sf)
  } else {
...
#=== make ab-lines data ===#
# used for creating ab-lines and/or the edge of the plot
mutate(ab_lines_data = list(
  make_ab_lines_data(exp_plots)
)) %>% 
#=== make ab-lines ===#
...
if (num_unique_plot_width == 1 & nrow(trial_data_pa) == 1) {
  ...
  #=== make ab-lines ===#
  # Note: ab-line can be different because of machine width
  mutate(ab_lines = list(
    ab_lines <- make_ab_lines(
      ab_lines_data = ab_lines_data,
      plot_width = input_plot_width, 
  ...
  mutate(exp_plots = list(
    make_trial_plots(
      ...
      ab_line = ifelse(
        ab_line_type == "lock",
        ab_sf, # ab-sf provided 
        line_edge # the edge of the experiment plots of the first input 
      ),
      plot_width = input_plot_width, 
      machine_width = machine_width,
      section_num = section_num, 
