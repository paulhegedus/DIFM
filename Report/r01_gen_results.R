...
g_opt_input <- data_sf %>% 
  ...
  scale_fill_brewer(
    palette = "Greens",
    direction = 1,
    name = "Optimal _input_name_here_c_ Rate \n (_unit_here_)"
  ) +
  theme_void() +
  theme(
...
sum_tab <- copy(opt_gc_data) %>% 
  ...
  setnames(names(.), c("Zone", "Strategy", "Yield (bu/acre)", "_input_name_here_c_ Rate")) %>% 
  flextable() %>% 
...
if (sn_ew_ratio > 1.1) {
  ...
  tm_tgti <- tm_shape(trial_design) +
    tm_fill(
      col = "tgti", 
      palette = "Greens",
      title = "Targeted _input_name_here_c_ Rate (_unit_here_)",
      legend.is.portrait = FALSE,
      style = "cat"
    ) +
    tm_layout_to_add + 
...
if (sn_ew_ratio > 1.1) {
 ...
 tm_input_aa <- tm_shape(input_plot_data) +
  tm_fill(
    col = "input_rate", 
    palette = "Greens",
    title = "As-applied _input_name_here_c_ Rate (_unit_here_)",
    legend.is.portrait = FALSE,
    style = "cont"
  ) +
  tm_layout_to_add + 
...
if (sn_ew_ratio > 1.1) {
  tm_p_input <- tm_shape(analysis_data_td) +
    tm_fill(
      col = "input_rate", 
      palette = "YlGn",
      title = "_input_name_here_c_ Rate\n(_unit_here_)",
      # legend.is.portrait = FALSE,
      style = "cont"
    ) +
    tm_layout_to_add + 
  ...
  tm_p_input <- tm_shape(analysis_data_td) +
    tm_fill(
      col = "input_rate", 
      palette = "YlGn",
      title = "_input_name_here_c_ Rate (_unit_here_)",
      legend.is.portrait = FALSE,
      style = "cont"
    ) +
    tm_layout_to_add + 
...
g_ys_zone_s <- ggplot() +
  ...
  facet_grid(. ~ zone_txt) +
  xlab("_input_name_here_c_ Rate (_unit_here_)") +
  ylab("Yield (bu/acre)") +
  theme(
    axis.title.x = element_text(size = 12, vjust = -1),
    axis.title.y = element_text(size = 12),
