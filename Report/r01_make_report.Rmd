...
```{r }
field_vars
field_var_len <- length(field_vars) 

field_vars[field_var_len] <- paste0("and ", field_vars[field_var_len]) 

field_var_txt <- paste0(field_vars, collapse = ", ")

#=== interacting field vars ===#
int_var_show <- field_plots$ch_var 
int_var_len <- length(int_var_show) 

int_var_show[int_var_len] <- paste0("and ", int_var_show[int_var_len]) 

int_var_txt <- paste0(int_var_show, collapse = ", ")
```

`r str_to_title(field_var_txt)` were tested to see if they are correlated with the management zones. Among these variables, `r int_var_txt` seem to affect how yield responds to _input_full_name_l_. 
