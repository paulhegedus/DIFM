...
+ Planter width: `r w_trial_info$machine_width` feet
...
The discussion thus far has offered statistical evidence of how yield responses and optimal management strategies differed among geographic areas of the field.  But no intuitive, agronomy-based explanations have been provided to explain the statistical results.  To investigate why some areas may have higher yield responses than others, it is necessary to examine whether field characteristics interact field characteristics with managed inputs in the yield response function.  Roughly speaking, if a field characteristic interacts with _input_full_name_l_ rate, it increases the slop of the yield-to-seed response curve.  
...
`r results$field_vars[[1]]` were tested to see if they are correlated with the management zones. Among these variables, `r field_plots$ch_var` seem to affect how yield responds to _input_full_name_l_. 

```{r fig.height = 5}

if (!is.null(field_plots)) {
  for (i in seq_len(nrow(field_plots))) {
    print(field_plots$g_map[[i]] | field_plots$g_ys_char[[i]])
  }
} 
