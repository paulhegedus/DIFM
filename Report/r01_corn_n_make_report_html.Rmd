...
```{r }
get_nitrogen <- function(c_type, w_zone){
  opt_gc_data[type == c_type & zone_txt == paste0("Zone ", w_zone), n_rate] %>% round(digits = 0)
}
...
trial_design <- mutate(trial_design, tgt_n = convert_N_unit(input_data_n$form, input_data_n$unit, tgt_n, field_data$reporting_unit))
...
ggplot() +
  geom_point(
    data = filter(
      data_sf, 
      n_rate >= min(eval_data$n_rate),
      n_rate <= max(eval_data$n_rate),
    ), 
