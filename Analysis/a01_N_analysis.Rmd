...
```{r data-prep-n, cache = F, results = "hide"}
...
```{r gam-estimation}
gam_res <- gam(
  yield ~ s(n_rate, k = 5, by = zone_txt) + s(X, k = 5) + s(Y, k = 5) + te(X, Y, k = c(5, 5)), 
  data = data_sf
) 
...
eval_data <- yield_shift[eval_data, on = "zone_txt"] %>% 
  ...
  .[, profit_hat := profit_hat + crop_price * yield_shift]  
...
pi_data_opt <- data_sf %>% 
  ...
  setnames("opt_n", "n_rate") %>% 
  predict_yield_pi_simple(., gam_res, "n_rate", crop_price) %>% 
