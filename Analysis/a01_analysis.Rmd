...
analysis_res_e <- analysis_res_gam %>% 
  ...
  mutate(
    ...
    mean_yield_hat_opt = 
    list(
      eval_data_opt[, .(mean_yield_hat = mean(yield_hat)), by = zone_txt]
    ), 
...
analysis_res_e %>% 
  mutate(
    ymin = eval_data_opt[, min(yield_hat - 1.96 * yield_hat_se)] - 10
  ) %>% 
  dplyr::select("data", "eval_data_opt", "ymin") %>% 
  mutate(g_ys = 
    list(
      eval_data_opt %>% 
      filter(type == "opt_v") %>% 
