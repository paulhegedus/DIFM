...
data_sf <- here("Data/Growers", ffy, "Analysis-Ready/analysis_data.rds") %>% 
  ...
  rename(
    yield = yield_vol,
    input_rate = input_name_here_rate,
    # input_rate = seed_rate,
  ) %>% 
  filter(!is.na(yield)) %>% 
  cbind(., st_coordinates(st_centroid(.)))  

input_price <- input_name_here_price
...
if (gc_type_s == "uniform") {
  ...
  gc_rate_data <- data.table(data_sf) %>% 
    unique(by = "zone") %>% 
    .[, input_rate := grower_chosen_rate_s] 
...
file_name <- here(
  "Reports/Growers", 
  ffy, 
  paste0("analysis_results_", input_name_here, ".rds")
)
