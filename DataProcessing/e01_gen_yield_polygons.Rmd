...
if(sum(ifelse(is.na(yield$distance) == FALSE, 1, 0)) > 1){
  dist_test <- yield %>%
...
if(sum(ifelse(is.na(yield$width) == FALSE, 1, 0)) > 1){
## we are checking the units of the width, we are assuming that the units in the field parameter are feet ##
if(median(yield$width, na.rm = TRUE) >= w_field_data$h_width[[1]]*.9 & median(yield$width, na.rm = TRUE) <= w_field_data$h_width[[1]]*1.1){
  units_width <- "ft"
}else{
  units_width <- "m"
}
