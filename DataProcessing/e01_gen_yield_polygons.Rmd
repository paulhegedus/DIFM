...
#--- yield ---#
yield <- here("Data/Growers", ffy) %>% 
  list.files(recursive = TRUE, full.names = TRUE) %>%
  .[str_detect(., "shp")] %>%
  .[str_detect(., "yield")] %>%
  st_read() 

if (is.na(st_crs(yield))) {
  yield <- st_set_crs(yield, 4326) 
} 
...
if (med_distance < 1){
  ...
  yield_reduced_v <- reduce_points_v(yield_grouped, num_obs_rg, "yield_vol", "group")
} else {
  yield_reduced_v <- data.table(yield_grouped)
}
```

+ before: `r nrow(yield_grouped)`
+ after: `r nrow(yield_reduced_v)`
...
`r length(overlap_id_ls)` were identified as polygons that visited previously visited area with significant overlap.
