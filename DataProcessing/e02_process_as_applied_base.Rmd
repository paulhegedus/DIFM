...
This code does not assume that `distance` and `width` have the same units, but test the units of both variables.

```{r}
if(sum(ifelse(is.na(aa_input$distance) == FALSE, 1, 0)) > 1){
  dist_test <- aa_input %>%
...
# we are checking the units of the width column and then replacing with width in field_parameter if missing
# we are assuming that the units in the field parameter are feet
input_data <- eval(parse(text = paste0("input_data_", tolower(input_type))))

if(sum(ifelse(is.na(aa_input$width) == FALSE, 1, 0)) > 1){
if(median(aa_input$width, na.rm = TRUE) >= input_data$machine_width*.9 & median(yield$width, na.rm = TRUE) <= input_data$machine_width*1.1){
  units_width <- "ft"
}else{
  units_width <- "m"
}
...
```{r }
if(is.na(units_distance) == TRUE & is.na(units_width) == TRUE){
  aa_input <- aa_input %>%
    mutate(
      width = conv_unit(input_data$machine_width, "ft", "m"),
      offset = conv_unit(offset, "ft", "m"))
}else if(is.na(units_distance) == FALSE & is.na(units_width) == FALSE){
  aa_input <- aa_input %>%
    mutate(
      width = conv_unit(width, units_width, "m"),
      distance = conv_unit(distance, units_distance, "m"),
      offset = conv_unit(offset, "ft", "m"))
}else if((is.na(units_distance) == FALSE & is.na(units_width) == TRUE)){
    aa_input <- aa_input %>%
    mutate(
      distance = conv_unit(distance, units_distance, "m"),
      width = conv_unit(input_data$machine_width, "ft", "m"),
      offset = conv_unit(offset, "ft", "m"))
}else if((is.na(units_distance) == TRUE & is.na(units_width) == FALSE)){
    aa_input <- aa_input %>%
    mutate(
      width = conv_unit(width, units_width, "m"),
      offset = conv_unit(offset, "ft", "m"))
