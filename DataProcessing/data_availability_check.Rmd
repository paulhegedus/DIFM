...
## Add "invalid_" to the directory names.

No datasets in the folders that start with **invalid_** are not going to be processed.
...
dir <- all_dirs[!is_dir_valid][1]

for (dir in all_dirs[!is_dir_valid]) {

  file.rename(
    paste0(here("Data", "Growers"), "/", dir), 
    paste0(here("Data", "Growers"), "/invalid_", dir) %>% gsub("invalid_", "", .) %>% gsub("Growers/", "Growers/invalid_", .)
  )
...
Here is the list of data directories that are invalid (missing information in `field_parameter.json`):

```{r }
all_dirs[!is_dir_valid]
```

First enter field information to `field_parameter.json`, run `00_generate_folder.R` and create the right folder structure for the field-year, and then place datasets where they should go.
...
field_year_ls <- field_data[, field_year] %>% unique
